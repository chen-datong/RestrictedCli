...
class FiniteField:
    ...
    def _generate_dual_basis(self):
        ...
        dual_basis = []
        for i in range(n):
            # Solve for b_i such that Tr(b_i * a^j) = δ_ij for j = 0, 1, ..., n-1
            b_i = self.field(_sage_const_0 )
            for j in range(n):
                b_i += self.field(self.p - _sage_const_1 ) * self.trace(a**j * a**i) * a**j
            dual_basis.append(b_i)
        return dual_basis
